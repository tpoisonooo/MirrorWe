...
def test_structure():
    ...
    try:
        ...
        # 检查关键组件
        key_components = [
            "class WeFactory:", "def get_person", "def get_group",
            "def get_entity", "_is_group_id", "_person_cache", "_group_cache",
            "def get_factory", "async def get_person", "async def get_group"
        ]
        ...
        # 检查LRU缓存机制
        cache_mechanisms = [
            "weakref.ref", "atexit.register", "_on_person_finalized",
            "_on_group_finalized", "_evict_oldest_person",
            "_evict_oldest_group"
        ]
        ...
        cleanup_functions = ["cleanup", "_cleanup_all"]
...
def test_group_id_detection():
    ...
    for test_id, expected, description in test_cases:
        ...
        if result == expected:
            print(
                f"✓ {description}: {test_id} -> {'Group' if result else 'Person'}"
            )
        else:
            print(
                f"✗ {description}: {test_id} -> 期望{'Group' if expected else 'Person'}, 实际{'Group' if result else 'Person'}"
            )
            all_passed = False
